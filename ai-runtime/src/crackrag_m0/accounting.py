...
def estimate_cost(usage: dict, pricing: Pricing, *, simulated: bool) -> dict:
    result = {
    ...
    components = {
        "input_fresh": Decimal(usage["input_fresh"]) * Decimal(pricing.input_miss_per_million) / million,
        "cache_read": Decimal(usage["cache_read"]) * Decimal(pricing.input_hit_per_million) / million,
        "output": Decimal(usage["output"]) * Decimal(pricing.output_per_million) / million,
    }
