...
class Pricing:
    ...
    def validate(self) -> None:
        if self.schedule not in ("fixed", "deepseek-cn-peak-v1"):
            raise ConfigError("unknown pricing schedule")
        if not isinstance(self.model, str):
            raise ConfigError("pricing.model must be a string")
        if self.schedule == "deepseek-cn-peak-v1" and (
            self.currency != "CNY" or self.model != "deepseek-flash"
        ):
            raise ConfigError("deepseek-cn-peak-v1 requires CNY and model deepseek-flash")
...
class Config:
    ...
    def validate(self) -> None:
        ...
        if self.provider != "mock" and self.pricing.model and self.model != self.pricing.model:
            raise ConfigError("pricing.model does not match the requested model")
