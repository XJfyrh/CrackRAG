...
class DeepSeekProvider:
    ...
    def __init__(self, config: Config, api_key: str, *, allow_live: bool = False,
        ...
        self.offline_transport = isinstance(transport, httpx.MockTransport)
        if not self.offline_transport and (config.model != 'deepseek-flash' or config.request.max_tokens > 512):
            raise ConfigError('M0 live calls require deepseek-flash and max_tokens <= 512')
        self.config = config
